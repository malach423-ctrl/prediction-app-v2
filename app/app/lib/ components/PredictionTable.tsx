...
export default function PredictionTable({ matches }: { matches: Match[] }) {
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 210px 150px 120px', padding: '0 0 6px', marginBottom: '4px' }}>
        {['Time', 'Match', '1 · X · 2', 'Goals O/U', 'Score'].map((h, i) => (
          <div key={h} style={{ fontSize: '10px', color: '#888780', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 500, textAlign: i >= 2 ? 'center' : 'left', paddingLeft: i === 0 ? '1rem' : i === 1 ? '1.25rem' : 0 }}>{h}</div>
        ))}
      </div>
      ...
      {matches.map((match) => {
        ...
        return (
          <div key={match.id} style={{ background: '#fff', border: '0.5px solid #d3d1c7', borderRadius: '12px', marginBottom: '10px', overflow: 'hidden' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 210px 150px 120px', alignItems: 'center' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', borderLeft: '0.5px solid #d3d1c7', display: 'flex', gap: 6, justifyContent: 'center' }}>
                {[
                  ...
                  const hot = value === max
                  return (
                    <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '6px 10px', borderRadius: '8px', minWidth: 48, background: hot ? '#eaf3de' : '#f4f3ef' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', borderLeft: '0.5px solid #d3d1c7', textAlign: 'center' }}>
                <div style={{ display: 'inline-block', background: '#e1f5ee', color: '#085041', fontFamily: 'var(--font-mono)', fontSize: '16px', fontWeight: 500, padding: '6px 14px', borderRadius: '8px', letterSpacing: '1px' }}>
                  {pred.correct_score}
                </div>
